    ...
    public static class BroadcastProcessor {
        public static void Weave(ModuleDefinition module, Dictionary<short, MethodDefinition> defs) {
            foreach (short key in defs.Keys) {
                ...
                processor.Append(processor.Create(OpCodes.Nop));
                processor.Append(processor.Create(OpCodes.Ldsfld, module.ImportReference(typeof(Server).GetField("Ins"))));
                processor.Append(processor.Create(OpCodes.Ldarg_0));
                processor.Append(processor.Create(OpCodes.Ldloc_0));
                processor.Append(processor.Create(OpCodes.Callvirt, module.ImportReference(typeof(Server).GetMethod("Broadcast", new Type[] { typeof(Connection), typeof(NetworkWriter) }))));
                processor.Append(processor.Create(OpCodes.Nop));
